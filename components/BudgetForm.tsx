    ...
    <Card>
      <form onSubmit={handleSubmit} className="p-6 sm:p-8 space-y-6">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div>
            <label htmlFor="city" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Your City</label>
            <Input id="city" name="city" type="text" value={budgetInput.city} onChange={handleInputChange} placeholder="e.g., New York" required />
          </div>
          <div>
            <label htmlFor="monthlyPreTaxIncome" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Monthly Pre-Tax Income ($)</label>
            ...
            <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">We'll estimate your after-tax income based on your city.</p>
          </div>
          <div className="md:col-span-2">
            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Monthly Savings Goal</label>
             <div className="flex items-center gap-x-6 gap-y-2 flex-wrap mb-2">
                <div className="flex items-center">
                    <input id="goal-percentage" name="savingsGoalType" type="radio" value="percentage" checked={budgetInput.savingsGoalType === 'percentage'} onChange={handleGoalTypeChange} className="h-4 w-4 text-sky-600 border-gray-300 dark:border-gray-600 focus:ring-sky-500 bg-transparent dark:bg-transparent dark:focus:ring-offset-gray-800" />
                    <label htmlFor="goal-percentage" className="ml-2 block text-sm text-gray-800 dark:text-gray-200">Percentage (%)</label>
                </div>
                <div className="flex items-center">
                    <input id="goal-amount" name="savingsGoalType" type="radio" value="amount" checked={budgetInput.savingsGoalType === 'amount'} onChange={handleGoalTypeChange} className="h-4 w-4 text-sky-600 border-gray-300 dark:border-gray-600 focus:ring-sky-500 bg-transparent dark:bg-transparent dark:focus:ring-offset-gray-800" />
                    <label htmlFor="goal-amount" className="ml-2 block text-sm text-gray-800 dark:text-gray-200">Fixed Amount ($)</label>
                </div>
            </div>
            <Input 
              id="savingsGoalValue" 
            ...
            {budgetInput.savingsGoalType === 'percentage' && (
              <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">Percentage is based on estimated after-tax income.</p>
            )}
          </div>
        </div>
        
        <div>
          <h3 className="text-lg font-semibold text-gray-800 dark:text-gray-100 border-b border-gray-200 dark:border-gray-700 pb-2 mb-4">Monthly Expenses</h3>
            ...
            {budgetInput.expenses.map((expense, index) => (
              <div key={expense.id} className="grid grid-cols-1 sm:grid-cols-8 gap-3 items-center">
                ...
                <div className="sm:col-span-1 flex justify-end">
                    <button type="button" onClick={() => removeExpense(expense.id)} className="p-2 text-gray-400 hover:text-red-500 hover:bg-gray-200 dark:hover:bg-gray-700 rounded-full transition-colors">
