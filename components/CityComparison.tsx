  ...
  return (
    <Card className={`flex flex-col h-full ${isBaseCity ? 'ring-2 ring-sky-500' : ''}`}>
      <div className="p-4 border-b border-gray-200 dark:border-gray-700 flex justify-between items-center">
        <h3 className="text-lg font-bold text-gray-900 dark:text-gray-50">{city}</h3>
        {isBaseCity ? (
          <span className="text-xs font-semibold bg-sky-100 text-sky-700 dark:bg-sky-900/50 dark:text-sky-300 px-2 py-1 rounded-full">Primary</span>
        ) : (
          <button onClick={onRemove} className="text-gray-400 hover:text-red-500 transition-colors" aria-label={`Remove ${city}`}>
      ...
      <div className="p-4 flex-grow space-y-4">
        ...
        {!isLoading && !isEditing && analysis && (
            <>
                <div className="space-y-2 text-sm">
                    <div className="flex justify-between"><span className="text-gray-600 dark:text-gray-400">After-Tax Income:</span> <span className="font-semibold text-green-600 dark:text-green-400">{formatCurrency(analysis.calculatedAfterTaxIncome)}</span></div>
                    <div className="flex justify-between"><span className="text-gray-600 dark:text-gray-400">Total Expenses:</span> <span className="font-semibold text-yellow-600 dark:text-yellow-400">-{formatCurrency(analysis.totalExpenses)}</span></div>
                    <div className="flex justify-between"><span className="text-gray-600 dark:text-gray-400">Savings:</span> <span className="font-semibold text-blue-600 dark:text-blue-400">-{formatCurrency(analysis.savingsAmount)}</span></div>
                    <div className="flex justify-between pt-2 border-t border-gray-200 dark:border-gray-700 mt-2 font-bold"><span className="text-sky-600 dark:text-sky-400">Disposable Income:</span> <span className="text-sky-600 dark:text-sky-400">{formatCurrency(analysis.disposableIncome)}</span></div>
                </div>
                <div className="text-xs space-y-1 mt-2 pt-2 border-t border-gray-200 dark:border-gray-700">
                    <h4 className="font-semibold text-gray-700 dark:text-gray-300 mb-1">Tax Breakdown</h4>
                    <div className="flex justify-between"><span className="text-gray-500 dark:text-gray-400">Federal Tax:</span> <span className="text-red-500">-{formatCurrency(analysis.taxBreakdown.federal)}</span></div>
                    <div className="flex justify-between"><span className="text-gray-500 dark:text-gray-400">State Tax:</span> <span className="text-red-500">-{formatCurrency(analysis.taxBreakdown.state)}</span></div>
                    <div className="flex justify-between"><span className="text-gray-500 dark:text-gray-400">Other Tax (FICA):</span> <span className="text-red-500">-{formatCurrency(analysis.taxBreakdown.other)}</span></div>
                </div>
            </>
        )}
        ...
        {!isBaseCity && !isLoading && isEditing && (
            <div className="space-y-4 text-sm">
                 <div>
                    <label className="text-sm font-medium text-gray-700 dark:text-gray-300">Est. Pre-Tax Income</label>
                 ...
                 <div>
                    ...
                    <div className="flex items-center gap-x-4 mb-1">
                        <div className="flex items-center"><input id={`goal-percentage-${city}`} type="radio" value="percentage" checked={savingsGoalType === 'percentage'} onChange={() => onDataChange(city, 'savingsGoalType', 'percentage')} className="h-4 w-4 text-sky-600 border-gray-300 dark:border-gray-600 bg-transparent"/> <label htmlFor={`goal-percentage-${city}`} className="ml-2 text-xs">Percent (%)</label></div>
                        <div className="flex items-center"><input id={`goal-amount-${city}`} type="radio" value="amount" checked={savingsGoalType === 'amount'} onChange={() => onDataChange(city, 'savingsGoalType', 'amount')} className="h-4 w-4 text-sky-600 border-gray-300 dark:border-gray-600 bg-transparent"/> <label htmlFor={`goal-amount-${city}`} className="ml-2 text-xs">Amount ($)</label></div>
                    </div>
                    ...
                    <div className="space-y-2 mt-1 max-h-40 overflow-y-auto pr-1">
                        {expenses.map(exp => (
                            <div key={exp.id} className="grid grid-cols-12 gap-2 items-center">
                                ...
                                <button type="button" onClick={() => removeExpense(city, exp.id)} className="col-span-1 text-gray-400 hover:text-red-500 justify-self-end">
      ...
      <div className="p-4 border-t border-gray-200 dark:border-gray-700 mt-auto">
        ...
        {!isLoading && !isEditing && analysis && (
            <p className="text-xs text-gray-500 dark:text-gray-400 italic">{analysis.summary}</p>
        )}
      </div>
    </Card>
  );
...
const CityComparison: React.FC<CityComparisonProps> = ({ baseCity, baseAnalysis, budgetInput }) => {
  ...
  return (
    <div className="space-y-8">
      <Card>
        <div className="p-6">
            <h2 className="text-2xl font-bold text-gray-900 dark:text-gray-50 mb-2">Compare Cost of Living</h2>
            <p className="text-gray-600 dark:text-gray-400 mb-4">Add up to 4 other cities to see how your budget stacks up.</p>
