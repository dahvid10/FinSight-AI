    ...
    <div id="results" className="scroll-mt-20">
      <Card>
        <div className="p-6 sm:p-8">
          <h2 className="text-3xl font-bold text-center mb-6 text-white">Your Personal Budget Analysis</h2>
            ...
            <TabPanel value="cashflow">
                <div className="max-w-md mx-auto">
                    ...
                    <div className="space-y-3 text-sm bg-gray-800/50 rounded-lg p-4 ring-1 ring-gray-700">
                        <div className="flex justify-between items-center"><span className="text-gray-400">Pre-Tax Income:</span> <span className="font-medium text-gray-100">{formatCurrency(preTaxIncome)}</span></div>
                        <div className="flex justify-between items-center pl-4 border-l-2 border-red-500/50"><span className="text-gray-400">Federal Tax:</span> <span className="font-medium text-red-400">-{formatCurrency(analysis.taxBreakdown.federal)}</span></div>
                        <div className="flex justify-between items-center pl-4 border-l-2 border-red-500/50"><span className="text-gray-400">State Tax:</span> <span className="font-medium text-red-400">-{formatCurrency(analysis.taxBreakdown.state)}</span></div>
                        <div className="flex justify-between items-center pl-4 border-l-2 border-red-500/50"><span className="text-gray-400">Other Taxes (FICA):</span> <span className="font-medium text-red-400">-{formatCurrency(analysis.taxBreakdown.other)}</span></div>
                        <div className="flex justify-between items-center font-bold border-t border-gray-700 pt-2 mt-2 !mb-2"><span className="text-green-400">After-Tax Income:</span> <span className="text-green-400">{formatCurrency(analysis.calculatedAfterTaxIncome)}</span></div>
                        ...
                        <div className="pl-8 space-y-1 my-1">
                          {expenses.filter(e => e.name && e.amount).map((expense) => (
                            <div key={expense.id} className="flex justify-between items-center">
                              ...
                              <span className="font-medium text-yellow-500/80 text-xs">-{formatCurrency(Number(expense.amount) || 0)}</span>
                            </div>
                          ))}
                        </div>
